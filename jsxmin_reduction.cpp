...
DEFINE_bool(code_reduction, true, "Enable code reduction optimization");

DEFINE_string(replace, "", "Format pattern:replacement, pattern:replacement");
DEFINE_bool(reduce, true, "Perform code reduction after replacement");

DECLARE_bool(debug);  // debug mode.

using namespace fbjs;
using namespace std;

void CodeReduction::process(NodeProgram* root) {
  // If the command line option disables codd reduction, skip the process.
  if (!FLAGS_code_reduction) {
    return;
  }
  ...
  if (!parse_patterns(FLAGS_replace)) {
    return;
  }
...
bool CodeReduction::parse_patterns(const string& input) {
  ...
  while (true) {
    ...
    if (p != string::npos) {
      ...
      cout << "// WARNING: ignore incomplete pattern " << pattern << "\n";
    }
